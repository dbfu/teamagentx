...
TeamAgentX 是一个多智能体协作平台，界面风格接近飞书 / Lark。用户可以在聊天室中通过 `@agent-name` 提及不同 AI 助手，让多个 agent 在同一个上下文中协作、交接任务、调用工具、维护房间记忆、创建待办事项，并执行定时任务。
