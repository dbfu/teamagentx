...
| **Smart Collaboration** | Fast-path relay (single-@ direct trigger) + 5-point coordinator fallback + collaboration-budget breakers + parallel-batch/serial-chain dispatch |
...
| **Collaboration Budget / Breaker** | Two breakers on single-@ relay: hops (20) / consecutive cycle (3 round-trips), counted between two human messages |
| **Parallel Batch / Serial Chain** | The coordinator splits multi-agent tasks per dispatchMode: parallel batch (fork-join, @s inside suspended to the join) or serial chain (advanced one by one by queue settlement events) |
