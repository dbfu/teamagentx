...
| **思考模式** | `off \| low \| medium \| high`，控制 Claude 系列模型的扩展思考（extended thinking）预算；默认 `high` |
...
| 模式 | 行为 | 适用场景 |
|------|------|---------|
| **智能协作（默认）** | 助手回复里写恰好一个合法 @ 走快路径直接接力（零协调成本）；只有用户路由落空 / @ 异常 / 批次汇合 / 卡住兜底 / 熔断这 5 个点才唤起内置「群调度助手」裁决；用户多 @ 交协调器拆为单任务/并行/串行 | 绝大多数多角色协作群 |
...
- "执行内容（发送的消息）" —— 到点向群里发送消息
- "选择要触发的助手" —— 多个助手会拆成多条消息逐个发送，每条消息只触发一个助手
- "最大重试次数"
- "触发用户" / "触发消息" / "执行时间" / "执行耗时"
...
18. **智能协作模式**（合并 auto/coordinator）+ 两重熔断防扇出/死循环（对应 A1/A2/H）
19. **群调度规则（工作流 YAML）+ 流程图可视化**
20. **并行批次 / 串行链派发**（协调器按 dispatchMode 选择）
21. **工作台「今日任务」与派发**
...
将外部 IM 平台的消息桥接到 TeamAgentX 群聊，消息流转与普通用户消息完全一致。

| 能力 | 状态 | 说明 |
|------|------|------|
| 创建机器人绑定 | ✅ | 在群聊设置中添加 BridgeBot，选平台填凭据 |
| Telegram | ✅ | 长轮询方式 |
| 飞书（Feishu） | ✅ | WebSocket 长连接 |
| 钉钉（DingTalk） | ✅ | Stream 长连接 |
| 企业微信（WeCom） | ✅ | Webhook 回调 |
| 内置群指令 | ✅ | `help`、`clear`（清空上下文）、`@助手名`（手动触发）|
| 配置向导 | ✅ | 每个平台有引导文案（Playbook）帮助填写 Token/Secret |

架构细节见 [08-server-architecture.md §10](08-server-architecture.md)。
...
| 能力 | 状态 | 说明 |
|------|------|------|
| TTS 文字转语音 | ✅ | 支持远端 TTS API |
| STT 语音识别 | ✅ | 可配置独立 STT 模型（`sttModel`）|
...
见 §4.4 与 [11-agent-trigger-system.md](11-agent-trigger-system.md)：快路径接力 + 协调器 5 点兜底 + 两重熔断 + 并行批次/串行链派发。全场景流程图见 [14-agent-dispatch-flowcharts.md](14-agent-dispatch-flowcharts.md)。
