...
export const llmProviderService = {
  async create(data: CreateLlmProviderInput): Promise<LlmProvider> {
    ...
    return prisma.llmProvider.create({
      data: {
        ...
        codexWireApi: data.codexWireApi || 'responses',
        apiUrl: data.apiUrl,
        apiKey: data.apiKey,
        model: data.model,
  ...
  // 导出用：返回完整（未脱敏）的供应商数据。传入 ids 则仅导出指定项
  async findForExport(ids?: string[]): Promise<LlmProvider[]> {
    return prisma.llmProvider.findMany({
  ...
  async update(id: string, data: UpdateLlmProviderInput): Promise<LlmProvider> {
    ...
    // 编辑时保留原有 API Key：未填写（空串/空白）或仍是遮罩值时，不覆盖数据库中的密钥
    if (
      'apiKey' in restData
