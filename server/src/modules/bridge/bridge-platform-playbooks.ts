...
export const BRIDGE_PLATFORM_PLAYBOOKS: BridgePlatformPlaybook[] = [
  {
    ...
    consoleSteps: [
      ...
      '在 @BotFather 发送 /mybots → 选择刚创建的 Bot → Bot Settings → Group Privacy → Turn off（关闭隐私模式，否则机器人无法读取群消息）',
      '将机器人拉入目标 Telegram 群',
    ],
    bindSteps: [
      '把 Bot Token 交给 TeamAgentX 接入助手保存',
      '在目标 TeamAgentX 房间生成绑定码',
      '在 Telegram 群内发送 /bind 绑定码，完成群聊映射',
    ],
    notes: [
      'Telegram 单条消息上限 4096 字符，系统会自动分段回发',
      '默认使用 Polling 轮询模式，无需公网地址；有公网地址时可在服务器配置 Webhook 以降低延迟',
    ],
  },
  {
    platform: 'feishu',
    ...
    consoleSteps: [
      '在飞书开放平台创建企业自建应用',
      '添加应用能力 → 机器人',
      '权限管理中申请 im:message 权限（覆盖群消息读写，无需单独申请子权限）',
      '事件与回调 → 选择「使用长连接接收事件」→ 添加 im.message.receive_v1 事件',
      '发布应用版本（内测发布即可）',
      '将应用机器人拉入目标飞书群',
    ],
    bindSteps: [
      '把 App ID 和 App Secret 交给 TeamAgentX 接入助手保存（无需配置公网地址）',
      '在目标 TeamAgentX 房间生成绑定码',
      '在飞书群内发送 /bind 绑定码，完成群聊映射',
    ],
    notes: [
  ...
  },
  {
    ...
    bindSteps: [
      '把 App Key 和 App Secret 交给 TeamAgentX 接入助手保存（无需配置公网地址）',
      '在目标 TeamAgentX 房间生成绑定码',
      '在钉钉群内 @机器人 发送 /bind 绑定码，完成群聊映射',
    ],
    notes: [
  ...
  },
  {
    ...
    consoleSteps: [
      ...
      '开启应用消息收发权限，将应用添加到目标企业微信群',
    ],
    bindSteps: [
      '把 Corp ID、Agent Secret、Token、EncodingAESKey 交给 TeamAgentX 接入助手保存',
      '在目标 TeamAgentX 房间生成绑定码',
      '在企业微信群内发送 /bind 绑定码，完成群聊映射',
    ],
    notes: [
  ...
  },
  {
    ...
    consoleSteps: [
      ...
      '开启 GROUP_AT_MESSAGE_CREATE 事件订阅（群 @机器人消息）',
      '开启群消息发送权限',
      '将 Bot 添加到目标 QQ 群并通过群主审批',
    ],
    bindSteps: [
      '把 App ID 和 Client Secret 交给 TeamAgentX 接入助手保存',
      '在目标 TeamAgentX 房间生成绑定码',
      '在 QQ 群内 @机器人 发送 /bind 绑定码，完成群聊映射',
    ],
    notes: [
      'QQ群与 QQ 频道是不同通道，这里只覆盖 QQ 群',
