...
- **agents** (必填): 数组，每项包含：
  - **agentId** (必填): 助手 ID
  - **speechConfig/name/description/prompt/llmProviderId/categoryId** (均可选)
...
2. 向用户展示将要修改的助手列表，确认后调用 update_agents（一次调用，串行执行）：
   {
     "agents": [
       { "agentId": "<id1>", "speechConfig": { "behavior": { "enabled": true, "outputMode": "auto_final_only", "autoPlay": false }, "profile": { "provider": "browser-local", "voice": null, "speed": 1, "volume": 1 } } },
       { "agentId": "<id2>", "speechConfig": { "behavior": { "enabled": true, "outputMode": "auto_final_only", "autoPlay": false }, "profile": { "provider": "browser-local", "voice": null, "speed": 1, "volume": 1 } } }
     ]
   }
