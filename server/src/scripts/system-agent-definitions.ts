...
- **voiceConfig** (可选): 语音播报配置，见下方说明

## voiceConfig 语音配置说明

用于配置助手的语音播报行为（浏览器端 TTS 朗读），字段如下：

- **enabled** (必填): 是否启用语音播报，true/false
- **outputMode** (必填): 播报模式
  - "off" - 关闭
  - "manual" - 手动播放（用户点击播放按钮）
  - "auto_final_only" - 自动播报最终回答（每次助手回答完成后自动朗读）
- **voiceId** (可选): 音色 ID，null 表示自动选择系统默认语音
- **speed** (可选): 语速，0.5-2，默认 1
- **volume** (可选): 音量，0-1，默认 1
- **autoPlay** (可选): 是否自动播放，默认 false
...
- **voiceId**: 不了解用户系统可用音色时一律设 null（自动选择），不要猜测音色名称
- **outputMode**: 用户主动要求"自动播报"时才设 auto_final_only；只说"开启语音"时设 manual
- **autoPlay**: 通常与 outputMode=auto_final_only 搭配，其余情况设 false
...
- **agents** (必填): 数组，每项包含：
  - **agentId** (必填): 助手 ID
  - **voiceConfig/name/description/prompt/llmProviderId/categoryId** (均可选)
...
2. 向用户展示将要修改的助手列表，确认后调用 update_agents（一次调用，串行执行）：
   {
     "agents": [
       { "agentId": "<id1>", "voiceConfig": { "enabled": true, "outputMode": "auto_final_only", "voiceId": null, "speed": 1, "volume": 1 } },
       { "agentId": "<id2>", "voiceConfig": { "enabled": true, "outputMode": "auto_final_only", "voiceId": null, "speed": 1, "volume": 1 } }
     ]
   }
...
2. 向用户展示配置："将为「辩论主持人」配置：自动播报、语速 1.3x，是否确认？"
