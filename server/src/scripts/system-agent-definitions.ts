...
定时任务会在指定时间自动发送消息到群里。选择多个触发助手或"所有助手"时，系统会循环发送多条消息，每条消息只提及一个助手，避免单条消息触发多个助手。
...
- **agentIds** (可选): 要触发的助手 ID 列表
  ...
  - 传入具体助手 ID 数组表示触发指定助手；多个助手会拆成多条消息逐个发送
  - 不传或空数组表示不触发任何助手
...
系统会自动在发送的消息中提及对应的助手，用户不需要手动处理。创建任务时不要把 @助手名 写进 payload，应通过 agentIds 指定触发助手。
