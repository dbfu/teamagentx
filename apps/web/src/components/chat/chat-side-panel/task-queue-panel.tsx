...
export function TaskQueuePanel({
  ...
  // 状态配置
  const statusConfig = {
    idle: { color: 'bg-muted', textColor: 'text-muted-foreground', label: t('agentStatus.idle') },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="text-muted-foreground">{t('taskQueue.agentStatusLabel')}：</span>
        <span className={cn(
      ...
      {agentInactiveTasks.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs text-orange-500">
            ...
            <span>{t('taskQueue.tasksToResume')}</span>
          </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm text-foreground truncate">
                  {task.messageContent || t('taskQueue.emptyMessage')}
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <span className={cn(
                    ...
                    {task.status === 'interrupted' ? t('taskQueue.serviceInterrupted') : t('taskQueue.cancelled')}
                  </span>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>{t('taskQueue.waitingTasks')}</span>
          ...
          {tasks.map((task, index) => {
            ...
            return (
              <div
                ...
                title={isFirstExecuting && onViewStream ? t('taskQueue.viewExecutionProcess') : undefined}
              >
                ...
                {!isFirstExecuting && (
                  <button
                    ...
                    title={t('taskQueue.cancelTaskTitle')}
                  >
                    <X className="size-3" />
                  </button>
                )}
