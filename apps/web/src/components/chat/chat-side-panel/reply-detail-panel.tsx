...
const mermaidCodeComponent = ({ className, children, ...props }: any) => {
...
export function ReplyDetailPanel({ selectedReplyMessage, replies, mentionAgents }: ReplyDetailPanelProps) {
  ...
  const originalSenderName = selectedReplyMessage.isHuman
    ? (selectedReplyMessage.user?.username ?? t('chat.user'))
    : (selectedReplyMessage.agent?.name ?? t('chat.assistant'))
        ...
        <div className="prose prose-sm [&_p]:whitespace-pre-wrap [&_li]:whitespace-pre-wrap">
          <ReactMarkdown
            ...
            components={{
              ...
              img: ({ src, alt }) => (
                <img src={resolveAssetUrl(src)} alt={alt || t('chat.image')} />
              ),
            }}
          >
            {content}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 text-muted-foreground text-xs">
        ...
        <span>{t('chat.replyDetail.repliesCount', { count: replies.length })}</span>
        <div className="flex-1 h-px bg-border" />
      </div>
      ...
      <div className="space-y-3">
        {replies.length === 0 ? (
          <div className="text-center text-muted-foreground py-4">
            {t('chat.replyDetail.noReplies')}
          </div>
        ) : (
          replies.map((reply) => {
            const replySenderName = reply.isHuman
              ? (reply.user?.username ?? t('chat.user'))
              : (reply.agent?.name ?? t('chat.assistant'))
            return (
