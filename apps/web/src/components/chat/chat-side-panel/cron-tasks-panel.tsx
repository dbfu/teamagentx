    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" onClick={() => setShowCreateModal(true)}>
            ...
            {t('chat.cronTasksPanel.createTask')}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-3">
          {loading ? (
            <div className="text-center text-muted-foreground py-8">{t('chat.cronTasksPanel.loading')}</div>
          ) : tasks.length === 0 ? (
            <div className="text-center text-muted-foreground py-8">
              {t('chat.cronTasksPanel.noCronTasksHint')}
            </div>
          ) : (
            tasks.map((task) => (
              <CronTaskCard
      ...
      <AlertDialog open={!!deletingTask} onOpenChange={() => setDeletingTask(null)}>
        <AlertDialogContent>
          <AlertDialogTitle>{t('chat.cronTasksPanel.deleteCronTask')}</AlertDialogTitle>
          <AlertDialogDescription>
            {t('chat.cronTasksPanel.deleteCronTaskConfirm', { name: deletingTask?.name })}
          </AlertDialogDescription>
          <AlertDialogFooter>
