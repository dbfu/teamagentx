...
export function AddAgentDialog({
  ...
  useEffect(() => {
    if (open) {
      setSearchQuery('')
      setSelectedAgents(new Set(initialSelectedIds))
        ...
        <div className="flex max-h-full w-[640px] flex-col overflow-hidden rounded-2xl bg-card shadow-xl">
          ...
          <div className="flex shrink-0 items-center justify-between border-b border-border px-6 py-4">
            <h2 className="text-lg font-semibold text-foreground">
              {t('chat.addAssistantDialogTitle')}
              {selectedAgents.size > 0 && (
                <span className="ml-2 text-sm text-muted-foreground">
                  ({t('chat.selectedCountShort', { count: selectedAgents.size })})
                </span>
              )}
            </h2>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {filteredAgents.length === 0 ? (
              <div className="text-sm text-muted-foreground py-4 text-center">
                {searchQuery.trim() ? t('chat.noMatchingAssistants') : t('chat.noAvailableAssistantsToAdd')}
              </div>
            ) : searchQuery.trim() ? (
              /* 搜索模式：扁平化网格展示 */
              ...
              <div className="space-y-3">
                ...
                {groupedAgents.categories.map((category) => (
                  <div key={category.id}>
                    ...
                    {expandedCategories.has(category.id) && (
                      <div className="grid grid-cols-5 gap-2 mt-1">{category.agents.map(renderAgent)}</div>
                    )}
                  </div>
                ))}
                ...
                {groupedAgents.uncategorized.length > 0 && (
                  <div>
                    ...
                    {expandedCategories.has('__uncategorized__') && (
                      <div className="grid grid-cols-5 gap-2 mt-1">{groupedAgents.uncategorized.map(renderAgent)}</div>
                    )}
                  </div>
                )}
              </div>
            ...
            >
              ...
              <span>
                {isAdding
                  ? t('chat.adding')
                  : confirmLabel
                    ? `${confirmLabel}${selectedAgents.size > 0 ? ` (${selectedAgents.size})` : ''}`
                    : t('chat.addWithCount', { count: selectedAgents.size > 0 ? ` (${selectedAgents.size})` : '' })}
              </span>
            </button>
          </div>
        </div>
