...
export function AssistantVoiceTab({ agent, onUpdate }: AssistantVoiceTabProps) {
  ...
  useEffect(() => {
    llmProviderApi.getAll().then((res) => {
      ...
      toast.error(t('assistant.voiceProviderLoadFailed'))
    })
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!response.success || !response.data) {
          throw new Error(t('assistant.voiceSaveFailed'))
        }
        ...
        if (requestId !== saveRequestIdRef.current) return
        const message = error instanceof Error ? error.message : t('assistant.voiceSaveFailed')
        setSaveState('error')
  ...
  const handlePreview = async () => {
    if (!voiceConfig.enabled) {
      toast.error(t('assistant.enableVoiceFirst'))
      return
    }
    setIsPreviewing(true)
    try {
      await speakText({
        text: t('assistant.voicePreviewText', { name: agent.name }),
      ...
      toast.error(error instanceof Error ? error.message : t('assistant.voicePreviewFailed'))
    } finally {
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card">
        <div className="border-b border-border bg-muted/50 px-6 py-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-foreground">{t('assistant.voiceSettings')}</h3>
          </div>
        </div>
        ...
        <div className="space-y-6 p-6">
          ...
          <div className="rounded-xl border border-border bg-muted/40 p-5">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-medium text-foreground">{t('assistant.enableVoiceBroadcast')}</p>
                  <p className="mt-1 text-sm text-muted-foreground">{t('assistant.enableVoiceBroadcastDesc')}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm text-muted-foreground">
                  {voiceConfig.enabled ? t('assistant.voiceEnabled') : t('assistant.voiceDisabled')}
                </span>
                <button
                  type="button"
                  role="switch"
                ...
                >
                  <span className={cn(
                    ...
                    voiceConfig.enabled ? 'translate-x-5' : 'translate-x-0',
                  )} />
                </button>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-border bg-card">
            <div className="border-b border-border px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-medium text-foreground">{t('assistant.broadcastMode')}</h4>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">{t('assistant.broadcastModeDesc')}</p>
            </div>
            <div className="p-5">
              <Select
                ...
                onValueChange={(value) => setVoiceConfig((prev) => ({
                  ...prev,
                  outputMode: value as AgentVoicePanelConfig['outputMode'],
                }))}
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={t('assistant.selectBroadcastMode')} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="off">{t('assistant.broadcastOff')}</SelectItem>
                  <SelectItem value="manual">{t('assistant.broadcastManual')}</SelectItem>
                  <SelectItem value="auto_final_only">{t('assistant.broadcastAutoFinal')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card">
            <div className="border-b border-border px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-medium text-foreground">{t('assistant.ttsSource')}</h4>
              </div>
              <p className="mt-1 text-sm text-muted-foreground">{t('assistant.ttsSourceDesc')}</p>
            </div>
            <div className="flex gap-3 p-5">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{t('assistant.browserLocal')}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">{t('assistant.browserLocalDesc')}</p>
                </div>
              </button>
              ...
              <button
                ...
                title={ttsProviders.length === 0 ? t('assistant.configureVoiceModelFirst') : undefined}
                className={cn(
              ...
              >
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">{t('assistant.remoteVoiceModel')}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    {ttsProviders.length === 0
                      ? t('assistant.configureVoiceModelFirst')
                      : t('assistant.availableProvidersCount', { count: ttsProviders.length })}
                  </p>
                </div>
              </button>
            </div>
            ...
            {isRemote && ttsProviders.length > 0 && (
              <div className="space-y-4 border-t border-border px-5 pb-5 pt-4">
                {/* 供应商选择 */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">{t('assistant.ttsProvider')}</label>
                  <select
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">{t('assistant.ttsModel')}</label>
                  <select
                ...
                {/* 音色选择 */}
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">{t('assistant.ttsVoice')}</label>
                  {voiceOptions && voiceOptions.length > 0 ? (
                    <select
                      ...
                      className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none disabled:opacity-60"
                    >
                      <option value="">{t('assistant.providerDefaultVoice')}</option>
                      {voiceOptions.map((v) => (
                    ...
                    <input
                      ...
                      value={voiceConfig.voiceId ?? ''}
                      onChange={(e) => setVoiceConfig((prev) => ({ ...prev, voiceId: e.target.value || null }))}
                      placeholder={t('assistant.providerDefaultVoiceHint')}
            ...
            {!isRemote && (
              <div className="border-t border-border p-5">
                <p className="mb-3 text-sm font-medium text-foreground">{t('assistant.builtInVoiceStyle')}</p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {active && (
                            <span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{t('assistant.currentStyle')}</span>
                          )}
                        </div>
                      </button>
                    )
                ...
                <div className="mt-4">
                  <label className="mb-1.5 block text-sm font-medium text-foreground">{t('assistant.localVoice')}</label>
                  {localVoices.length > 0 ? (
                    <Select
                      value={voiceConfig.voiceId ?? '__default__'}
                      disabled={!voiceConfig.enabled}
                      onValueChange={(value) => setVoiceConfig((prev) => ({
                        ...prev,
                        voiceId: value === '__default__' ? null : value,
                      }))}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={t('assistant.systemDefaultVoice')} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__default__">{t('assistant.systemDefaultVoice')}</SelectItem>
                    ...
                    <input
                      ...
                      value={voiceConfig.voiceId ?? ''}
                      onChange={(e) => setVoiceConfig((prev) => ({ ...prev, voiceId: e.target.value || null }))}
                      placeholder={t('assistant.noLocalVoiceHint')}
                      className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none disabled:opacity-60"
                    />
                  )}
                </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="font-medium text-foreground">{t('assistant.previewAndSaveStatus')}</p>
                <p className="mt-1 text-sm text-muted-foreground">{t('assistant.previewAndSaveDesc')}</p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                ...
                <div className="min-h-5 text-sm">
                  ...
                  {saveState === 'error' && (
                    <span className="text-red-500">{saveError || t('common.saveFailed')}</span>
                  )}
                </div>
              </div>
            </div>
