...
export function AssistantHistoryTab({ agentId }: AssistantHistoryTabProps) {
  ...
  if (!user?.id) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-sm text-muted-foreground">登录后可以查看会话记录</p>
      </div>
    )
  }
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-foreground">快速对话会话</h2>
      ...
      {sessions.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 bg-muted/50 rounded-2xl border border-border">
          ...
          <h3 className="text-lg font-medium text-foreground mb-2">暂无会话记录</h3>
          <p className="text-sm text-muted-foreground">点击"快速对话"按钮创建新会话</p>
        </div>
      ) : (
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="font-medium text-foreground">
                        {session.chatRoom.name || `会话 #${index + 1}`}
                      </h4>
                      <Badge
