...
export function TodoModal({ isOpen, onClose, onTodoClick }: TodoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4">
      ...
      <div className="relative z-10 flex max-h-[80vh] w-full max-w-[640px] flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-semibold text-foreground">{t('chat.todoTitle')}</h3>
              </div>
              <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                <span className="rounded-full border border-border bg-background px-2 py-0.5">
                  {t('chat.todoPendingCount', { count: pendingTodos.length })}
                </span>
              </div>
            </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto bg-background/35">
          {pendingTodos.length === 0 ? (
            <div className="flex min-h-[260px] flex-col items-center justify-center gap-3 px-6 text-center text-muted-foreground">
              ...
              <div>
                <div className="text-sm font-medium text-foreground">{t('chat.noTodos')}</div>
                <div className="mt-1 text-xs">{t('chat.noTodosHint')}</div>
              </div>
            </div>
          ) : (
