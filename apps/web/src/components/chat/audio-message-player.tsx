    ...
    <div className={cn('w-full max-w-md rounded-2xl border border-border bg-card p-3 shadow-sm', className)}>
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          title={isPlaying ? t('common.pause') : t('chat.play')}
          aria-label={isPlaying ? t('common.pause') : t('chat.play')}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-2">
            <input
              ...
              aria-label={t('chat.playProgress')}
            />
      ...
      {transcriptText ? (
        ...
        <p className="mt-2 text-xs text-muted-foreground/60">{t('chat.notTranscribed')}</p>
      )}

      {hasPlaybackError && (
        <div role="alert" className="mt-2 text-xs text-red-500">
          {t('chat.audioPlayFailed')}
        </div>
      )}
    </div>
  )
