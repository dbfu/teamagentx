...
export function ScreenshotModal({
  ...
  const generateScreenshot = useCallback(async () => {
    ...
    try {
      ...
      const message = t('toast.screenshotGenerateFailed')
      setError(message)
      toast.error(t('toast.screenshotGenerateFailed'))
    } finally {
      setGenerating(false)
    }
  ...
  const pageStatusText = useMemo(() => {
    ...
    return t('chat.screenshotPageStatus', { current: currentPageIndex + 1, total: pageCount })
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      toast.success(hasMultiplePages ? t('toast.screenshotPageDownloaded', { page: currentPageIndex + 1 }) : t('toast.screenshotDownloaded'))
      if (!hasMultiplePages) {
        onOpenChange(false)
      }
    } catch {
  ...
  const handleCopy = useCallback(async () => {
    ...
    if (success) {
      setCopied(true)
      toast.success(hasMultiplePages ? t('toast.screenshotPageCopied', { page: currentPageIndex + 1 }) : t('toast.copiedToClipboard'))
      setTimeout(() => setCopied(false), 2000)
    } else {
      toast.error(t('toast.copyFailedAlt'))
    }
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      toast.success(result.method === 'electron' ? t('toast.pdfExported') : t('toast.pdfPrintOpened'))
    } catch (err) {
      console.error('导出 PDF 失败:', err)
      toast.error(t('toast.pdfExportFailed'))
    } finally {
      setPrintingPdf(false)
    }
  }, [roomName])
      ...
      >
        ...
        >
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">{t('toast.noMessagesToScreenshot')}</h3>
            <p className="text-muted-foreground mb-4">{t('toast.noMessagesHint')}</p>
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-semibold">{t('chat.screenshotTitle')}</h3>
            </div>
            <button
            ...
            <div className="border rounded-lg bg-muted/30 p-4 min-h-[200px] max-h-[400px] overflow-auto">
              {generating && (
                <div className="flex items-center justify-center h-[200px]">
                  ...
                  <span className="ml-2 text-muted-foreground">{t('chat.screenshotGenerating')}</span>
                </div>
              )}
              ...
              {!generating && !error && pdfOnlyMode && (
                <div className="flex h-[200px] flex-col items-center justify-center rounded-lg border border-blue-100 bg-blue-50 px-4 text-center text-blue-700">
                  ...
                  <div className="text-sm font-medium">{t('chat.pdfRecommended')}</div>
                  <div className="mt-1 text-xs">
                    {t('chat.pdfEstimateHint', { count: estimatedImagePageCount })}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-3">
                  {hasMultiplePages && (
                    <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700">
                      <span>{t('chat.screenshotSplitHint', { count: pageCount })}</span>
                      <div className="flex items-center gap-2">
                        <Button
                          type="button"
                          variant="outline"
                  ...
                  <img
                    src={currentPage.url}
                    alt={hasMultiplePages ? t('chat.screenshotPreviewPageAlt', { page: currentPageIndex + 1 }) : t('chat.screenshotPreviewAlt')}
          ...
          <div className="flex items-center justify-end gap-2 px-6 py-4 border-t">
            ...
            >
              {printingPdf ? (
                <><Loader2 className="size-4 mr-1 animate-spin" /> {t('chat.pdfGenerating')}</>
              ) : (
                <><FileText className="size-4 mr-1" /> {t('chat.exportPdf')}</>
              )}
            </Button>
            <Button
              variant="outline"
            ...
            >
              {copied ? (
                <><Check className="size-4 mr-1" /> {t('chat.copiedAlt')}</>
              ) : (
                <><Copy className="size-4 mr-1" /> {hasMultiplePages ? t('chat.copyCurrentPage') : t('common.copy')}</>
              )}
            </Button>
            <Button
            ...
            >
              ...
              {hasMultiplePages ? t('chat.downloadCurrentPage') : t('chat.downloadImage')}
            </Button>
          </div>
        </div>
      </div>
