...
export function SidebarNav({ messageBadge }: SidebarNavProps) {
  ...
  const navItemConfig: Record<MainNavTab, { icon: LucideIcon; label: string }> = {
    ...
    connectors: { icon: Plug, label: 'MCP' },
  ...
  const brandLogoSrc = isElectron ? './app-logo.png' : '/app-logo.png'
  ...
  const navTabs: Array<{ id: OptionalNavTab; icon: LucideIcon; label: string; menuLabel: string }> = [
    ...
    { id: 'connectors', icon: Plug, label: 'MCP', menuLabel: 'MCP' },
  ...
  const renderMeasuredNavGroup = (hiddenTabs: OptionalNavTab[]) => {
    ...
    <div className={cn("flex w-full flex-col items-center gap-1 px-2 pb-4 select-none", navTopPaddingClass)}>
      <div className="mb-2 flex size-10 items-center justify-center self-center overflow-hidden rounded-xl border border-sidebar-border bg-[var(--surface-raised)] shadow-[var(--control-shadow)]">
        <img src={brandLogoSrc} alt="TeamAgentX" draggable={false} className="size-full object-cover" />
      </div>
      <button tabIndex={-1} className={cn("group flex w-full items-center justify-center rounded-lg py-1 text-muted-foreground", isElectron && "mb-2")}>
  ...
  return (
    ...
    >
      ...
      <div ref={navItemsRef} className={cn("flex min-h-0 w-full flex-1 flex-col items-center gap-1 px-2 pb-4 select-none", navTopPaddingClass)}>
        <div className="mb-2 flex size-10 shrink-0 items-center justify-center self-center overflow-hidden rounded-xl border border-sidebar-border bg-[var(--surface-raised)] shadow-[var(--control-shadow)]">
          <img src={brandLogoSrc} alt="TeamAgentX" draggable={false} className="size-full object-cover" />
        </div>
        <button
          onClick={() => setIsSearchOpen(true)}
          className={cn("group flex w-full cursor-pointer items-center justify-center rounded-lg py-1 text-muted-foreground transition-colors hover:bg-sidebar-accent hover:text-foreground", isElectron && "mb-2")}
      ...
      <div className="flex shrink-0 flex-col items-center gap-1 pb-4">
        {isElectron && updateState.update && (
          <button
            ...
            onClick={() => updateManager.openNotification('sidebar')}
