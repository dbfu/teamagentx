...
export function RegisterModal({ isOpen, onRegister }: RegisterModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username.length < 2 || username.length > 20) {
      setError(t('auth.usernameLengthRange'))
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/50 p-4">
      <div className="w-[420px] max-w-full shrink-0 max-h-[80vh] overflow-y-auto rounded-2xl bg-card shadow-xl">
        {/* Header */}
        <div className="flex items-center gap-3 border-b border-border px-6 py-4">
          ...
          <h2 className="text-lg font-semibold text-foreground">{t('auth.registerTitle')}</h2>
        </div>
